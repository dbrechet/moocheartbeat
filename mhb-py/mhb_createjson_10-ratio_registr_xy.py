...
while d <= max_i:
    ...
    for row_s in rows_session:
        ...
        rcnttot = rcnttot + float(row_s[1])
        if nvrow != 0:
            sessions.append (row_s[0]+'_item')
            regs.append (mktime(d.timetuple()))
            sessions.append(row_s[0]+'_activity')
            regs.append (round(nvrow,2))
            sessions.append(row_s[0]+'_registr')
            regs.append (round(rcnttot,2))
...
with io.open('mhb-data/10_courses-ratio-registr.json', 'w', encoding='utf-8') as f:
    f.write(unicode(json.dumps(data_dict, sort_keys=True, indent=4, separators=(',', ': '), ensure_ascii=False)))

jsfilename ='mhb-js/10_courses-ratio-registr.js'
jsmessage = """
...
htmlfilename = '10_test-courses-ratio-registr.html'
htmlmessage = """
    <!DOCTYPE html>
    <html>
	   <head>
            ...
            <script type="text/javascript" src="mhb-js/10_courses-ratio-registr.js"></script>
