...
for row_session in rows_session:
    ...
    #sql_item = """SELECT item, registrations, assignment_unique + forum_comment_unique + forum_post_unique + lecture_download_unique + lecture_view_unique + quiz_exam_unique + quiz_homework_unique + quiz_quiz_unique + quiz_survey_unique + quiz_video_unique as activity, assignment_unique, assignment_total/assignment_unique as assignment_ratio, forum_comment_unique, forum_comment_total/forum_comment_unique as forum_comment_ratio, forum_post_unique, forum_post_total/forum_post_unique as forum_post_ratio, lecture_download_unique, lecture_download_total/lecture_download_unique as lecture_download_ratio, lecture_view_unique, lecture_view_total/lecture_view_unique as lecture_view_ratio, quiz_exam_unique, quiz_exam_total/quiz_exam_unique as quiz_exam_ratio, quiz_homework_unique, quiz_homework_total/quiz_homework_unique as quiz_homework_ratio, quiz_quiz_unique, quiz_quiz_total/quiz_quiz_unique as quiz_quiz_ratio, quiz_survey_unique,  quiz_survey_total/quiz_survey_unique as quiz_survey_ratio, quiz_video_unique, quiz_video_total/quiz_video_unique as quiz_video_ratio
#                      FROM mhb_coursera 
#                      WHERE session='"""+data_session+"' ORDER BY item ASC"
    
    sql_regs = """SELECT item, registrations, assignment_unique + forum_comment_unique + forum_post_unique + lecture_download_unique + lecture_view_unique + quiz_exam_unique + quiz_homework_unique + quiz_quiz_unique + quiz_survey_unique + quiz_video_unique as activity
    FROM mhb_coursera 
    WHERE session='"""+data_session+"' ORDER BY item ASC"
    
    cursor.execute(sql_regs)
    rows = cursor.fetchall()
    columns = list( [d[0].decode('utf8') for d in cursor.description] )
    colname = columns[1]
    columns[0] = colname + '_item'
    columns[1] = data_mooc + '_registrations'
    columns[2] = data_mooc + '_activityunique'
    columns.append(data_mooc + '_session') #3
    columns.append(data_mooc + '_date') #4
    columns.append(data_mooc + '_mooc') #5
    ...
    for row in rows:
        ...
        regs = regs + row[1]
        row[1] = regs
        row[2] = round(float(row[2]/float(regs)), 2)
    ...
    datafilename = 'mhb-data/19_onecourse_'+data_session+'_uniqueregsum_activity_xy.json'
    ...
    # create the js file.
    jsfilename ='mhb-js/19_onecourse_'+data_session+'_uniqueregsum_activity_xy.js'
    jsmessage = """
        var data = d3.json("mhb-data/19_onecourse_"""+data_session+"""_uniqueregsum_activity_xy.json", function(error, data){
    ...
    htmlfilename = '19_test-onecourse_'+data_session+'_uniqueregsum_activity_xy.html'
    htmlmessage = """
    <!DOCTYPE html>
    <html>
	   <head>
            <title>19 uniqueregsum_activity_xy """+data_session+""" | amCharts</title>
            ...
            <script type="text/javascript" src="mhb-js/19_onecourse_"""+data_session+"""_uniqueregsum_activity_xy.js"></script>
