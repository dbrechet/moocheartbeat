#!/usr/bin/env python
#title           :mhb_createjson_onecourse_uniqueregsum_activity.py
...
#date            :20141014
#version         :0.1
#usage           :python mhb_createjson_onecourse_uniqueregsum_activity.py
#notes           :
...
for row_session in rows_session:
    ...
    sql_item = """SELECT item, registrations, assignment_unique + forum_comment_unique + forum_post_unique + 
                      ...
                      (assignment_unique + forum_comment_unique + forum_post_unique + 
                     lecture_download_unique + lecture_view_unique + quiz_exam_unique + quiz_homework_unique + 
                      quiz_quiz_unique + quiz_survey_unique + quiz_video_unique + work_combined_unique)/13 as mean_activity
                      FROM mhb_coursera 
                      WHERE session='"""+data_session+"' ORDER BY item ASC"
    ...
    datafilename = 'mhb-data/07_onecourse_'+data_session+'_uniqueregsum_activity.json'
    ...
    jsfilename ='mhb-js/07_onecourse_'+data_session+'_uniqueregsum_activity.js'
    jsmessage = """
        var data = d3.json("mhb-data/07_onecourse_"""+data_session+"""_uniqueregsum_activity.json", function(error, data){
        var chart = AmCharts.makeChart("chartdiv", data);
        });"""
    with io.open(jsfilename, 'w', encoding='utf-8') as fj:
        fj.write(unicode(jsmessage))
    htmlfilename = '07_test-onecourse_'+data_session+'_uniqueregsum_activity.html'
    htmlmessage = """
    <!DOCTYPE html>
    <html>
	   <head>
            <title>uniqueregsum_activity """+data_session+""" | amCharts</title>
            ...
            <script type="text/javascript" src="mhb-js/07_onecourse_"""+data_session+"""_uniqueregsum_activity.js"></script>
