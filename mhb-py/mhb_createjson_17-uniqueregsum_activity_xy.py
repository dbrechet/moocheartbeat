...
for row_session in rows_session:
    ...
    sql_item = """SELECT item, registrations, assignment_total + forum_comment_total + forum_post_total + 
                      ...
                      quiz_quiz_total + quiz_survey_total + quiz_video_total as activity
                      FROM mhb_coursera 
                      WHERE session='"""+data_session+"' ORDER BY item ASC"
    ...
    columns[2] = data_mooc + '_activity'
    columns.append(data_mooc + '_session') #3
    columns.append(data_mooc + '_date') #4
    columns.append(data_mooc + '_actrel') #5
    columns.append(data_mooc + '_mooc') #6
    ...
    if data_mooc_old != data_mooc:
        ...
        item_graph_value.append("Name:<b>[["+columns[3]+"]]</b><br>Date:<b>[["+columns[4]+"]]</b><br>Regs:<b>[["+columns[1]+"]]</b><br>Act:<b>[["+columns[2]+"]]</b><br>ActRel:<b>[["+columns[5]+"]]</b>")
        ...
        item_graph_value.append(columns[5])
...
datafilename = 'mhb-data/17_uniqueregsum_activity_xy.json'
...
jsfilename ='mhb-js/17_uniqueregsum_activity_xy.js'
jsmessage = """
    var data = d3.json("mhb-data/17_uniqueregsum_activity_xy.json", function(error, data){
...
htmlfilename = '17_test-uniqueregsum_activity_xy.html'
htmlmessage = """
<!DOCTYPE html>
<html>
    <head>
        <title>Iter 07: 17, uniqueregsum_activity_xy| amCharts</title>
        ...
        <script type="text/javascript" src="mhb-js/17_uniqueregsum_activity_xy.js"></script>
