#!/usr/bin/env python
#title           :mhb_createjson_onecourse_total.py
...
#date            :20141014
#version         :0.1
#usage           :python mhb_createjson_onecourse_total.py
#notes           :
...
for row_session in rows_session:
    ...
    # dataProvider
    sql_item = """SELECT item, registrations, assignment_total, forum_comment_total, forum_post_total, lecture_combined_total, 
                      lecture_download_total, lecture_view_total, quiz_exam_total, quiz_homework_total, quiz_quiz_total, quiz_survey_total,
                      quiz_video_total
                      FROM mhb_coursera 
    ...
    datafilename = 'mhb-data/03_onecourse_'+data_session+'_total.json'
    ...
    jsfilename ='mhb-js/03_onecourse_'+data_session+'_total.js'
    jsmessage = """
        var data = d3.json("mhb-data/03_onecourse_"""+data_session+"""_total.json", function(error, data){
        var chart = AmCharts.makeChart("chartdiv", data);
        });"""
    with io.open(jsfilename, 'w', encoding='utf-8') as fj:
        fj.write(unicode(jsmessage))
    htmlfilename = '03_test-onecourse_'+data_session+'_total.html'
    htmlmessage = """
    <!DOCTYPE html>
    <html>
	   <head>
            <title>Iter02: 03, Total """+data_session+""" | amCharts</title>
            ...
            <script type="text/javascript" src="mhb-js/03_onecourse_"""+data_session+"""_total.js"></script>
