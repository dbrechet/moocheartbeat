#!/usr/bin/env python
#title           :mhb_createjson_onecourse_uniqueregsum_nostack.py
...
#date            :20141014
#version         :0.1
#usage           :python mhb_createjson_onecourse_.py
#notes           :
...
for row_session in rows_session:
    ...
    datafilename = 'mhb-data/08_onecourse_'+data_session+'_uniqueregsum_nostack.json'
    ...
    jsfilename ='mhb-js/08_onecourse_'+data_session+'_uniqueregsum_nostack.js'
    jsmessage = """
        var data = d3.json("mhb-data/08_onecourse_"""+data_session+"""_uniqueregsum_nostack.json", function(error, data){
        var chart = AmCharts.makeChart("chartdiv", data);
        });"""
    with io.open(jsfilename, 'w', encoding='utf-8') as fj:
        fj.write(unicode(jsmessage))
    htmlfilename = '08_test-onecourse_'+data_session+'_uniqueregsum_nostack.html'
    htmlmessage = """
    <!DOCTYPE html>
    <html>
	   <head>
            <title>uniqueregsum_nostack """+data_session+""" | amCharts</title>
            ...
            <script type="text/javascript" src="mhb-js/08_onecourse_"""+data_session+"""_uniqueregsum_nostack.js"></script>
